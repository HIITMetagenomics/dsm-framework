#ifndef SUCCINCTVECTOR_H
#define SUCCINCTVECTOR_H
...
class SuccinctVector : public BitVector
{
  public:
    ...
    SuccinctVector(std::ifstream& file);
    SuccinctVector(Encoder& encoder, usint universe_size);
    SuccinctVector(Encoder& encoder); // Use the array directly.
    SuccinctVector(WriteBuffer& vector);
    ~SuccinctVector();
...
//--------------------------------------------------------------------------
    ...
    {
      public:
        Iterator(const SuccinctVector& par);
        ~Iterator();
