...
/*
  This is a temporary program for experimenting subset compression in document listing.
*/
...
//  #endif
  ...
  // Store the document array for other uses.
  std::string doc_name = base_name + ".docs";
  std::ofstream doc_file(doc_name.c_str(), std::ios_base::binary);
  if(doc_file)
  {
    std::cout << "Storing document array in " << doc_name << std::endl;
    doc_file.write((char*)documents, size * sizeof(usint));
    doc_file.close();
  }
  ...
  root->range.second = size - 1; root->contains_all = true;
  delete[] lcp; lcp = 0;
  ...
  while(nodestack.top() != root)
  {
    ...
    std::vector<uint> vec(documents + curr->range.first, documents + curr->range.second + 1);
    removeDuplicates(&vec, (vec.size() >= 65536));
    if(curr->child != 0) { curr->computeStoredDocuments(); }
    if(vec.size() >= docs)
    {
      for(STNode* temp = curr; temp != 0; temp = temp->parent) { temp->contains_all = true; }
    }

    if(curr->child == 0 || curr->contains_all || curr->stored_documents > storing_factor * vec.size())
    {
      curr->stored_documents = vec.size();
      ...
      if(!(curr->contains_all))
      {
        ...
        if(vec.size() == 1)
        {
          singletons << (-curr_node) << ": " << vec[0] << std::endl;
        }
        else
        {
          output.write((char*)&curr_node, sizeof(curr_node));
          for(std::vector<uint>::iterator iter = vec.begin(); iter != vec.end(); ++iter)
          {
            uint temp = *iter;
...
testDocarray(const std::string& base_name)
{
  ...
  DocArray docarray(base_name, rlcsa, true);
...
buildIndex(const std::string& base_name, const std::string& prefix)
{
  RLCSA rlcsa(base_name);
  if(!(rlcsa.isOk())) { return 4; }
  DocArray docarray(base_name, rlcsa, false);
  if(!(docarray.isOk())) { return 5; }
