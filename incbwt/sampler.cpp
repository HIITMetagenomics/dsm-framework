...
bool
WeightedSampler::buildSamples(uint sample_rate, sum_type initial_adjustment)
{
  ...
  while(true)
  {
    ...
    shortest = this->minimumWeightPath(true);
    ...
    longest = this->minimumWeightPath(false);
...
uint
WeightedSampler::minimumWeightPath(bool shortest)
{
  ...
  node_type result = this->link(active_nodes, this->nodes - 1, shortest);
