...
RLCSA::readBWT(pair_type range) const
{
  ...
  std::cerr << "rlcsa::readBWT() allocating n = " << n << " bytes" << std::endl;
  uchar* bwt = new uchar[n];
  std::cerr << "allocation successful.. zeroing" <<  std::endl;
  memset(bwt, 0, n);
  std::cerr << "zeroing successful.. computing bwt" <<  std::endl;

  for(usint c = 0; c < CHARS; c++)
  {
    if(this->array[c] != 0)
    {
      std::cerr << "iteration " << c <<  std::endl;
  ...
  std::cerr << "all iterations done " << std::endl;
